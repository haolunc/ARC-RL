...
LLM evaluation pipeline for [ARC-AGI-2](https://github.com/arcprize/ARC-AGI-2) tasks using the Responses API. Three evaluation modes:
...
| `tree_search` | Yes | Our tool loop | MCTS-style search over multiple solution attempts with PUCT selection and refinement. |
...
├── tree.py         # MCTS-style solution search
...
run.py  main()
 ...
 └── evaluate_single_task(task_id, task_data, ...)
      ...
      ├── tree.py       TreeSearch(...).run() → best code, when mode=tree_search
