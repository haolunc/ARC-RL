...
from arc.eval.config import MAX_TOOL_ROUNDS, WARN_THRESHOLD, TEMPERATURE, EXEC_TIMEOUT, MAX_OUTPUT_TOKENS, TOKEN_BUDGET
...
def call_llm(
    client, cfg: dict, messages: list[dict], task_id: str, log_db=None,
    *, max_tool_rounds: int | None = None, max_output_tokens: int | None = None,
    ...
    """
    _max_tool_rounds = max_tool_rounds if max_tool_rounds is not None else MAX_TOOL_ROUNDS
    _max_output_tokens = max_output_tokens if max_output_tokens is not None else MAX_OUTPUT_TOKENS
    _token_budget = token_budget if token_budget is not None else TOKEN_BUDGET
    ...
    for round_num in range(_max_tool_rounds):
        remaining = _max_tool_rounds - round_num
        ...
        if round_num > 0:
            if remaining <= 1:
                ...
                content = TOOL_CALL_WARNING.format(remaining=remaining)
                input_list.append({"role": "user", "content": content})
                transcript_parts.append(f"[User]\n{content}")

        print(f"[{task_id}] Round {round_num+1}/{_max_tool_rounds}: calling API...")
        response = _api_call(
            client,
            task_id=task_id,
            model=model,
        ...
        # Incremental log save after each tool round
        if log_db:
            log_db.save_log(task_id, "\n\n".join(transcript_parts), raw_responses)

        # Token budget check
        if usage["input"] >= _token_budget:
            error = f"token budget exceeded ({usage['input']} >= {_token_budget})"
            print(f"[{task_id}] {error}")
            break

    else:
        # for-else: loop exhausted without break = exceeded max rounds
