...
from arc.eval.tree import TreeSearch
...
def evaluate_single_task(task_id, task_data, client, cfg, db, log_db):
    ...
    try:
        ...
        if mode == "tree_search":
            tree = TreeSearch(task_id, task_data, client, cfg, log_db)
            best_code = tree.run()
            tree_result = tree.build_result()
            extracted_code = best_code

            base_result.update({
                "token_usage": json.dumps(tree_result["token_usage"]),
                "tool_rounds": tree_result["tool_rounds"],
            ...
            if extracted_code is None:
                base_result.update({
                    ...
                    "correct": 0,
                    "extracted_code": None,
                    "error_message": "No valid code found in tree search",
                })
                db.save_result(base_result)
                return base_result

            print(f"[{task_id}] Running tests with best tree search solution...")
            test_result = run_tests(extracted_code, task_data["test"], cfg["python_path"])
            print(f"[{task_id}] Tests: {test_result['passed']}/{test_result['total']}, "
            ...
            if log_db:
                log_db.save_log(task_id, tree.build_log_text(), tree_result["raw_responses"])
        ...
        if llm_result.error:
            print(f"[{task_id}] LLM warning: {llm_result.error} (continuing with extraction)")
        ...
        if extracted_code is None:
            ...
            base_result.update({
                ...
                "correct": 0,
                "extracted_code": None,
                "error_message": llm_result.error or "No test_transform function found in LLM output",
            })
            db.save_result(base_result)
            return base_result
